...
function ReportsExport({ role = "admin" }) {
    ...
    api.get('/api/billing/history')
      .then(res => {
        const data = (res.data || []).map(b => ({
          ...
          productName: (b.items && b.items[0]?.productName) || '',
          quantity: b.totalItems || 0,
          revenue: b.grandTotal || 0,
          gst: b.gstTotal || 0,
          discount: b.discountTotal || 0,
  ...
  const filteredSales = useMemo(() => {
    ...
    return sales.filter((sale) => {
      ...
      const matchPayment = paymentFilter === "All" ||
        String(sale.paymentStatus || sale.payment || "").toUpperCase() === paymentFilter.toUpperCase();
      const matchSearch = !searchTerm ||
        String(sale.invoiceNumber || "").toLowerCase().includes(lowerSearch) ||
        String(sale.customerName || "").toLowerCase().includes(lowerSearch) ||
        String(sale.productName || "").toLowerCase().includes(lowerSearch);
  ...
  const totalGst = filteredSales.reduce((sum, s) => sum + Number(s.gst || 0), 0);
      ...
      .map(
        (sale) => `
          <tr>
            ...
            <td>${money(sale.revenue || 0)}</td>
            <td>${money(sale.gst || 0)}</td>
        ...
        <div className="rep-card">
          <div className="rep-table-wrap">
            <table className="rep-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan="9" className="rep-empty">Loading...</td></tr>
                ) : filteredSales.map((sale, index) => (
                  <tr key={sale.id || index}>
                    <td>{sale.date || "-"}</td>
                    <td style={{ fontWeight: 500, color: "#2D2D2D" }}>{sale.invoiceNumber || "-"}</td>
                    <td>{sale.customerName || "-"}</td>
                    <td style={{ fontSize: 12, color: "#3F3F46" }}>{sale.productName || "-"}</td>
                    <td style={{ fontWeight: 500, color: "#2D2D2D" }}>{sale.quantity || 0}</td>
                    <td style={{ fontWeight: 600, color: "#2D2D2D" }}>{money(sale.revenue || 0)}</td>
                    <td style={{ color: "#8B7355" }}>{money(sale.gst || 0)}</td>
                    <td style={{ color: "#9B4444" }}>{money(sale.discount || 0)}</td>
                    <td>
                      <span className="rep-badge">{sale.payment || sale.paymentStatus || "-"}</span>
                    </td>
                  </tr>
                ))}
