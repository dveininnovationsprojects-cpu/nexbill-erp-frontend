...
const MOCK_PRODUCTS = [
  { id: 1, sku: 'SKU001', name: 'Wireless Mouse',   price: 599,  gstRate: 18, category: 'Electronics' },
  { id: 2, sku: 'SKU002', name: 'Rice 5kg',          price: 280,  gstRate: 5,  category: 'Groceries' },
  { id: 3, sku: 'SKU003', name: 'Blue Pen Pack',     price: 45,   gstRate: 12, category: 'Stationery' },
  { id: 4, sku: 'SKU004', name: 'Cotton T-Shirt',    price: 399,  gstRate: 5,  category: 'Clothing' },
  { id: 5, sku: 'SKU005', name: 'Mineral Water 1L',  price: 20,   gstRate: 0,  category: 'Beverages' },
  { id: 6, sku: 'SKU006', name: 'Notebook A4',       price: 85,   gstRate: 12, category: 'Stationery' },
  { id: 7, sku: 'SKU007', name: 'USB-C Cable',       price: 249,  gstRate: 18, category: 'Electronics' },
  { id: 8, sku: 'SKU008', name: 'Sunflower Oil 1L',  price: 180,  gstRate: 5,  category: 'Groceries' },
];
...
export default function CashierBilling() {
  ...
  const [paymentMethod, setPaymentMethod] = useState(null); // cash|upi|card|netbanking
  ...
  const openInvoice = () => { setPaymentMethod(null); setPaymentStatus(null); setInvoiceModal(true); };

  const handlePayment = () => {
    if (!paymentMethod) return;
    setPaymentStatus('processing');
    // TODO: connect to /payment/create-order API
  ...
  return (
    <>
      <style>{`
        ...
        .bill-invoice{background:#FFFFFF;border-radius:18px;width:100%;max-width:420px;box-shadow:0 20px 60px rgba(45,45,45,0.2);overflow:hidden}
        .bill-invoice-header{background:#2D2D2D;padding:24px;text-align:center;color:#F8F5F2}
        ...
        .bill-invoice-actions{display:flex;gap:10px;padding:0 24px 24px}
              ...
              {paymentStatus === 'processing' ? (
                ...
                <>
                  ...
                  <div className="pay-methods">
                    {[{id:'cash',label:'Cash'},{id:'upi',label:'UPI'},{id:'card',label:'Card'},{id:'netbanking',label:'Net Banking'}].map(m => (
                      <button
                        key={m.id}
            ...
            {filtered.map(p => {
              ...
              return (
                <div key={p.id} className={`bill-product-card ${inCart ? 'in-cart' : ''}`} onClick={() => addToCart(p)}>
                  <div className="bill-p-cat">{p.category}</div>
                  <div className="bill-p-name">{p.name}</div>
                  <div className="bill-p-sku">{p.sku}</div>
                  <div className="bill-p-price">₹{p.price.toLocaleString()}</div>
                  <div className="bill-p-gst">GST: {p.gstRate}%</div>
