// ╔══════════════════════════════════════════════════════════════════════╗
// ║   NexBill ERP — Profile & Logout Module                             ║
// ║   Includes: Profile Overview, Edit Info, Role Details,             ║
// ║             Security, Activity Log, Logout Confirmation            ║
...
/* ══════════════════════════════════════════════════════════════════════
   STYLES — NexBill Design System (matches Settings & Invoices)
══════════════════════════════════════════════════════════════════════ */
const STYLES = `
  .pr-shell {
    display: flex;
  ...
  .pr-avatar {
    ...
    border: 3px solid rgba(198,169,105,0.3);
  ...
  .pr-avatar-edit:hover { background: #EFE7DE; }
  .pr-hero-name {
  ...
  .pr-hero-stats {
    ...
    gap: 10px; width: 100%; position: relative; z-index: 1;
  }
  .pr-hero-stat {
  ...
  .pr-info-row:last-child { border-bottom: none; padding-bottom: 0; }
  .pr-info-icon {
  ...
  .pr-activity-item:last-child { border-bottom: none; padding-bottom: 0; }
  .pr-act-dot {
  ...
  .pr-btn-danger:hover { background: #dc2626; color: #FFFFFF; }
  .pr-btn-logout {
    display: flex; align-items: center; gap: 8px;
    padding: 0 20px; height: 40px; width: 100%;
  ...
  .pr-btn-logout:hover { opacity: 0.9; }
  ...
  .pr-toast-err { background: #7A3A3A; }
  @keyframes prSlideIn {
  ...
  .pr-logout-modal {
    ...
    padding: 32px 28px; width: 380px; max-width: 95vw;
    box-shadow: 0 20px 60px rgba(45,45,45,0.25);
  ...
  .pr-logout-user-email { font-size: 13px; font-weight: 600; color: #2D2D2D; }
  .pr-logout-user-role  { font-size: 11px; color: #8B7355; margin-top: 2px; }
  .pr-logout-actions    { display: flex; gap: 10px; }
  .pr-logout-actions .pr-btn-secondary { flex: 1; justify-content: center; }
  .pr-logout-actions .pr-btn-logout    { flex: 1; }
...
/* ══════════════════════════════════════════════════════════════════════
   MOCK DATA — Activity log & sessions
══════════════════════════════════════════════════════════════════════ */
const ACTIVITY_LOG = [
  { dot: 'green', title: 'Logged in successfully',   meta: 'Chrome · Windows · 192.168.1.10',         time: 'Just now'    },
  { dot: 'gold',  title: 'Invoice #INV-1042 created', meta: 'Admin created new invoice for Ravi Kumar',  time: '2h ago'     },
...
const CASHIER_ACTIVITY = [
  ...
  { dot: 'blue',  title: 'Product stock checked',       meta: 'Checked 5 items for low stock',            time: '4h ago'     },
...
function Toggle({ on, onChange }) {
  return (
    <button className={`pr-toggle-btn ${on ? 'pr-toggle-on' : 'pr-toggle-off'}`} onClick={() => onChange(!on)}>
    ...
    <div className="pr-overlay" onClick={onClose}>
      <div className="pr-pw-modal" onClick={e => e.stopPropagation()}>
        <div className="pr-pw-modal-head">
          ...
          <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#8B7355', display: 'flex', alignItems: 'center' }} onClick={onClose}>
            <X size={18} />
          </button>
        </div>
        <form onSubmit={handleSubmit} className="pr-pw-form">
          ...
          <div style={{ background: '#F8F5F2', borderRadius: 9, padding: '10px 14px', fontSize: 12, color: '#8B7355' }}>
    ...
    <div className="pr-overlay" onClick={onCancel}>
      <div className="pr-logout-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="pr-logout-desc">
          You're about to sign out of your account. Any unsaved changes will be lost. You'll need to sign in again to access the system.
        </div>
        <div className="pr-logout-user">
          ...
          <div>
            ...
            <div className="pr-logout-user-role">{user?.role === 'ADMIN' ? '⬡ Administrator' : '⬡ Cashier'}</div>
          </div>
        </div>
        <div className="pr-logout-actions">
          ...
          <button className="pr-btn-logout" onClick={onConfirm} disabled={loading}>
            {loading ? <span className="pr-spinner" style={{ borderTopColor: '#FFFFFF' }} /> : <><LogOut size={15} /> Sign Out</>}
          </button>
        </div>
      </div>
    </div>
...
══════════════════════════════════════════════════════════════════════ */
function PersonalInfoSection({ user, onSave }) {
    ...
    <div className="pr-card">
      <div className="pr-card-head">
        <div>
          ...
          <div className="pr-card-sub">Your display name, contact, and bio details</div>
        </div>
        {!editing && (
          <button className="pr-btn-secondary" style={{ height: 34, fontSize: 12 }} onClick={() => setEditing(true)}>
            <Edit3 size={13} /> Edit
          </button>
        )}
      </div>
          ...
          <>
            ...
            <div className="pr-field">
              ...
              <textarea value={form.bio} onChange={e => set('bio', e.target.value)} rows={3}
                style={{ padding: '9px 12px', border: '1.5px solid #EFE7DE', borderRadius: 9, fontSize: 13, color: '#2D2D2D', background: '#F8F5F2', outline: 'none', fontFamily: 'inherit', resize: 'vertical', width: '100%' }} />
            </div>
          </>
        ) : (
          <>
            <div className="pr-info-row" style={{ paddingTop: 0 }}>
              <div className="pr-info-icon"><User size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Full Name</div>
                <div className="pr-info-val">{form.name}</div>
              </div>
            </div>
            <div className="pr-info-row">
              <div className="pr-info-icon"><Mail size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Email Address</div>
                <div className="pr-info-val">{user?.email}</div>
              </div>
              <div style={{ fontSize: 11, fontWeight: 600, color: '#16a34a', background: '#DCFCE7', padding: '3px 8px', borderRadius: 12 }}>Verified</div>
            </div>
            <div className="pr-info-row">
              <div className="pr-info-icon"><Phone size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Phone Number</div>
                <div className="pr-info-val">{form.phone}</div>
              </div>
            </div>
            <div className="pr-info-row">
              <div className="pr-info-icon"><Briefcase size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Department</div>
                <div className="pr-info-val">{form.department}</div>
              </div>
            </div>
            <div className="pr-info-row">
              <div className="pr-info-icon"><MapPin size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Location</div>
                <div className="pr-info-val">{form.location}</div>
              </div>
            </div>
            <div className="pr-info-row">
              <div className="pr-info-icon" style={{ height: 'auto', width: 32 }}><Star size={15} /></div>
              <div className="pr-info-content">
                <div className="pr-info-label">Bio</div>
                <div className="pr-info-val" style={{ fontWeight: 400, color: '#3F3F46', lineHeight: 1.5 }}>{form.bio}</div>
              </div>
            </div>
          </>
        )}
    ...
    <div className="pr-card">
      <div className="pr-card-head">
        <div>
          ...
          <div className="pr-card-sub">Your admin privileges and access level</div>
        </div>
      </div>
      <div className="pr-card-body">
        <div className="pr-role-panel">
          ...
          <div className="pr-rp-grid">
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Access Level</div>
              <div className="pr-rp-stat-val">Level 5</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Since</div>
              <div className="pr-rp-stat-val">Jan 2024</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Users Managed</div>
              <div className="pr-rp-stat-val">12</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Last Action</div>
              <div className="pr-rp-stat-val">2h ago</div>
            </div>
          </div>
        </div>
        ...
        {[
          ...
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid #F8F5F2' }}>
            <span style={{ fontSize: 13, color: '#2D2D2D', fontWeight: 500 }}>{p.label}</span>
            {p.granted
              ? <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: '#16a34a', background: '#DCFCE7', padding: '3px 9px', borderRadius: 12 }}><CheckCircle size={11} /> Granted</span>
              : <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 700, color: '#8B7355', background: '#F1EEE8', padding: '3px 9px', borderRadius: 12 }}>Restricted</span>
            }
          </div>
        ))}
      </div>
...
/* ══════════════════════════════════════════════════════════════════════
   CASHIER ROLE INFO SECTION
══════════════════════════════════════════════════════════════════════ */
function CashierRoleSection({ user }) {
  const [data] = useState({
    branch:        'Main Branch — Bangalore',
    counter:       'Counter 2',
    shift:         '9:00 AM – 5:00 PM',
    salary:        '₹18,000 / month',
    employeeId:    'NB-CSH-042',
    ...
    <div className="pr-card">
      ...
      <div className="pr-card-body">
        <div className="pr-role-panel">
          ...
          <div className="pr-rp-grid">
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Today's Bills</div>
              <div className="pr-rp-stat-val">{data.todayInvoices}</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">This Week</div>
              <div className="pr-rp-stat-val">{data.weekInvoices}</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Revenue</div>
              <div className="pr-rp-stat-val" style={{ fontSize: 12 }}>{data.totalRevenue}</div>
            </div>
            <div className="pr-rp-stat">
              <div className="pr-rp-stat-label">Avg. Bill</div>
              <div className="pr-rp-stat-val" style={{ fontSize: 12 }}>{data.avgBillValue}</div>
            </div>
          </div>
        </div>
        ...
        {[
          { icon: Hash,       label: 'Employee ID',    val: data.employeeId   },
          { icon: Building2,  label: 'Branch',         val: data.branch       },
          { icon: Briefcase,  label: 'Counter',        val: data.counter      },
          { icon: Clock,      label: 'Shift Timing',   val: data.shift        },
          { icon: Calendar,   label: 'Joining Date',   val: data.joiningDate  },
          { icon: User,       label: 'Supervisor',     val: data.supervisor   },
          { icon: TrendingUp, label: 'Basic Salary',   val: data.salary       },
        ].map(({ icon: Icon, label, val }, i) => (
          <div key={i} className="pr-info-row" style={i === 0 ? { paddingTop: 0 } : {}}>
...
══════════════════════════════════════════════════════════════════════ */
function SecuritySection({ onOpenChangePW, onSave }) {
    ...
    <div className="pr-card">
      <div className="pr-card-head">
        <div>
          ...
          <div className="pr-card-sub">Password, 2FA, and login preferences</div>
        </div>
      </div>
      <div className="pr-card-body">
        ...
        {twoFA && (
          <div style={{ background: '#DCFCE7', border: '1px solid #86EFAC', borderRadius: 9, padding: '10px 14px', fontSize: 12, color: '#15803d', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
            <CheckCircle size={13} /> 2FA active — your account is secured with an authenticator app.
          </div>
        )}

        <div className="pr-sec-item">
          <div className="pr-sec-icon"><Activity size={16} /></div>
          <div className="pr-sec-info">
        ...
        <div className="pr-section-lbl"><Monitor size={11} /> Active Sessions</div>
        {SESSIONS.map((s, i) => (
      ...
      <div className="pr-card-foot">
        <button className="pr-btn-secondary" onClick={() => onSave('All sessions revoked')}>
          <RefreshCw size={13} /> Revoke All Sessions
        </button>
      </div>
    </div>
  );
...
══════════════════════════════════════════════════════════════════════ */
function ActivitySection({ isAdmin }) {
    ...
    <div className="pr-card">
      <div className="pr-card-head">
        <div>
          ...
          <div className="pr-card-sub">Your login history and recent actions</div>
        </div>
        <button className="pr-btn-secondary" style={{ height: 32, fontSize: 12 }} onClick={() => setShowAll(v => !v)}>
  ...
  return (
    <div className="pr-danger-zone">
      ...
      <div className="pr-danger-body">
        <div className="pr-danger-item" style={{ paddingTop: 0 }}>
          <div>
            <div className="pr-danger-label">Sign Out All Devices</div>
            <div className="pr-danger-desc">Immediately end all active sessions on all devices</div>
          </div>
          <button className="pr-btn-danger" onClick={onLogout}>
            <LogOut size={13} /> Sign Out All
          </button>
        </div>
        <div className="pr-danger-item">
          <div>
            <div className="pr-danger-label">Deactivate Account</div>
            <div className="pr-danger-desc">Temporarily disable your account (contact admin to restore)</div>
          </div>
          <button className="pr-btn-danger">
            <AlertCircle size={13} /> Deactivate
          </button>
        </div>
      </div>
    </div>
...
export default function Profile() {
  ...
  return (
    <>
      ...
      {/* ── Toast ── */}
      {toast && (
        <div className={`pr-toast ${toast.type === 'error' ? 'pr-toast-err' : ''}`}>
      ...
      <div className="pr-shell">
        ...
        <div className="pr-left">
          ...
          <div className="pr-card">
            ...
            <div className="pr-card-body" style={{ padding: '12px 16px' }}>
              {[
                { icon: Calendar, label: 'Member Since', val: 'Jan 2024'            },
                { icon: Clock,    label: 'Last Login',   val: 'Today, 9:14 AM'      },
                { icon: Globe,    label: 'Timezone',     val: 'IST (UTC+5:30)'      },
                { icon: Wifi,     label: 'IP Address',   val: '192.168.1.10'        },
              ].map(({ icon: Icon, label, val }, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: i < 3 ? '1px solid #F8F5F2' : 'none' }}>
                  <div style={{ width: 28, height: 28, background: '#EFE7DE', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#8B7355', flexShrink: 0 }}>
                    <Icon size={13} />
                  </div>
                  <div>
                    <div style={{ fontSize: 10, color: '#8B7355', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.4px' }}>{label}</div>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#2D2D2D', marginTop: 1 }}>{val}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="pr-card">
            ...
            <div style={{ padding: '8px 10px' }}>
              <button className="pr-quick-action" onClick={() => setShowCPW(true)}>
                ...
                <div><div className="pr-qa-label">Change Password</div><div className="pr-qa-sub">Update your password</div></div>
              ...
              <button className="pr-quick-action" onClick={() => navigate(isAdmin ? '/admin/settings' : '/cashier/settings')}>
                <div className="pr-qa-icon pr-qa-blue"><Globe size={15} /></div>
                <div><div className="pr-qa-label">Settings</div><div className="pr-qa-sub">System preferences</div></div>
                <ChevronRight size={14} color="#D6D3D1" />
              </button>
              ...
              <button className="pr-quick-action" onClick={() => setShowLogout(true)} style={{ marginTop: 4 }}>
        ...
        <div className="pr-right">
          ...
          {/* Personal Info */}
          <PersonalInfoSection user={user} onSave={showToast} />

          {/* Role-specific section */}
          {isAdmin
            ? <AdminRoleSection />
            : <CashierRoleSection user={user} />
          }
          ...
          <div className="pr-card">
            <div className="pr-card-head">
              <div>
                <div className="pr-card-title"><LogOut size={15} /> Sign Out</div>
                <div className="pr-card-sub">End your current session and return to the login screen</div>
              </div>
            </div>
            <div className="pr-card-body">
              <div style={{ background: '#F8F5F2', border: '1px solid #EFE7DE', borderRadius: 10, padding: '14px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: '#8B7355', marginTop: 2 }}>{isAdmin ? 'Administrator · NexBill ERP' : 'Cashier · NexBill ERP'}</div>
                </div>
              </div>
              <p style={{ fontSize: 13, color: '#8B7355', lineHeight: 1.6, margin: '0 0 16px' }}>
                Signing out will end your session. You will need to log in again to access the dashboard. Make sure all pending work is saved before signing out.
              </p>
              <button className="pr-btn-logout" onClick={() => setShowLogout(true)}>
                <LogOut size={16} /> Sign Out of NexBill ERP
              </button>
            </div>
          </div>

          {/* Danger Zone */}
          <DangerZoneSection onLogout={() => setShowLogout(true)} />
